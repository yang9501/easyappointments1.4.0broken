...
(function (exports) {
    ...
    function bindEventHandlers() {
        ...
        $calendar.on('click', '.edit-popover', function () {
            ...
            if (lastFocusedEventData.data.workingPlanException) {
                ...
                $dialog.find('#appointment-question1').val(appointment.question1);
                $dialog.find('#appointment-question2').val(appointment.question2);
                $dialog.find('#appointment-question3').val(appointment.question3);
                $dialog.find('#appointment-question4').val(appointment.question4);
                $dialog.find('#appointment-question5').val(appointment.question5);
    ...
    /**
     * Get the event question1 for the popup widget.
     *
     * @param {Event} event
     */
    function getEventQuestion1(event) {
        if (!event.data || !event.data.question1) {
            return '-';
        }

        var question1 = event.data.question1;

        return question1.length > 100 ? question1.substring(0, 100) + '...' : question1;
    }

    /**
     * Get the event question2 for the popup widget.
     *
     * @param {Event} event
     */
    function getEventQuestion2(event) {
        if (!event.data || !event.data.question2) {
            return '-';
        }

        var question2 = event.data.question2;

        return question2.length > 100 ? question2.substring(0, 100) + '...' : question2;
    }

    /**
     * Get the event question3 for the popup widget.
     *
     * @param {Event} event
     */
    function getEventQuestion3(event) {
        if (!event.data || !event.data.question3) {
            return '-';
        }

        var question3 = event.data.question3;

        return question3.length > 100 ? question3.substring(0, 100) + '...' : question3;
    }

    /**
     * Get the event question4 for the popup widget.
     *
     * @param {Event} event
     */
    function getEventQuestion4(event) {
        if (!event.data || !event.data.question4) {
            return '-';
        }

        var question4 = event.data.question4;

        return question4.length > 100 ? question4.substring(0, 100) + '...' : question4;
    }

    /**
     * Get the event question5 for the popup widget.
     *
     * @param {Event} event
     */
    function getEventQuestion5(event) {
        if (!event.data || !event.data.question5) {
            return '-';
        }

        var question5 = event.data.question5;

        return question5.length > 100 ? question5.substring(0, 100) + '...' : question5;
    }
        ...
        if ($(this).hasClass('fc-unavailable') || $parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
            ...
            $html = $('<div/>', {
                'html': [
                    ...
                    $('<strong/>', {
                        'text': 'Bring own firearms?  '
                    }),
                    $('<span/>', {
                        'text': getEventQuestion1(event)
                    }),
                    $('<br/>'),
